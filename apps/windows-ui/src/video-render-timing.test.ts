...
describe("video render timing", () => {
  test("does not report an fps before two displayed frames", () => {
    ...
    timing.observe(100);
    expect(timing.snapshot(1_000)).toEqual({ displayedFpsX100: null, maxFrameGapMs: null });
  });
  ...
  test("reports a bounded average fps and the worst frame gap", () => {
    ...
    timing.observe(133);
    timing.observe(300);
    expect(timing.snapshot(1_100)).toEqual({ displayedFpsX100: 300, maxFrameGapMs: 167 });
  });
  ...
  test("ignores invalid timestamps and resets between streams", () => {
    ...
    timing.reset();
    expect(timing.snapshot(30)).toEqual({ displayedFpsX100: null, maxFrameGapMs: null });
  });
  ...
  test("caps pathological gaps and rates", () => {
    ...
    expect(timing.snapshot(1_000_001)).toEqual({ displayedFpsX100: 0, maxFrameGapMs: 60_000 });
  });
});
