...
/// Marker proving that Desklink's audited zeroization patch is selected.
pub const DESKLINK_ZEROIZE_PATCH: &str = "blake2-0.10.6-desklink-zeroize-v1";
...
mod desklink_zeroize_tests {
    use super::{Blake2sVarCore, DESKLINK_ZEROIZE_PATCH};
    use zeroize::ZeroizeOnDrop;

    #[test]
    fn blake2s_compression_state_zeroizes_on_drop() {
        fn assert_zeroize_on_drop<T: ZeroizeOnDrop>() {}

        assert_zeroize_on_drop::<Blake2sVarCore>();
        assert_eq!(DESKLINK_ZEROIZE_PATCH, "blake2-0.10.6-desklink-zeroize-v1");
    }
}
