...
impl<'builder> Builder<'builder> {
    ...
    pub fn generate_keypair(&self) -> Result<Keypair, Error> {
        ...
        let mut private = Zeroizing::new(vec![0u8; dh.priv_len()]);
    ...
    fn build(self, initiator: bool) -> Result<HandshakeState, Error> {
        ...
        let cipher = self
            ...
            .ok_or(InitStage::GetCipherImpl)?;
        let hash = self
            .resolver
            .resolve_hash(&self.params.hash)
            .ok_or(InitStage::GetHashImpl)?;
        let mut s_dh = self
            .resolver
            .resolve_dh(&self.params.dh)
            .ok_or(InitStage::GetDhImpl)?;
        ...
        let mut psks: [Option<Zeroizing<[u8; PSKLEN]>>; 10] = std::array::from_fn(|_| None);
        for (i, psk) in self.psks.iter().enumerate() {
            if let Some(key) = *psk {
                if key.len() != PSKLEN {
                    return Err(InitStage::ValidatePskLengths.into());
                }
                let mut k = Zeroizing::new([0u8; PSKLEN]);
    ...
    fn resolve_kem(
        ...
        if hs
            ...
            .modifiers
            .list
            .contains(&HandshakeModifier::Hfs)
        {
            if let Some(kem_choice) = hs.params.kem {
                let kem = resolver
                    .resolve_kem(&kem_choice)
                    .ok_or(InitStage::GetKemImpl)?;
                hs.set_kem(kem);
            } else {
...
mod tests {
    ...
    fn test_partialeq_impl() {
        let keypair_1 = Keypair {
            private: Zeroizing::new(vec![0x01; 32]),
            public: vec![0x01; 32],
        };
        ...
        // Wrong private
        keypair_2.private = Zeroizing::new(vec![0x50; 32]);
        assert_eq!(keypair_1 == keypair_2, false);
        // Reset to original
        keypair_2.private = Zeroizing::new(vec![0x01; 32]);
        // Wrong public
