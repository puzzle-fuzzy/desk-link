...
impl HandshakeState {
    ...
    fn dh(&self, token: &DhToken) -> Result<Zeroizing<[u8; MAXDHLEN]>, Error> {
        let mut dh_out = Zeroizing::new([0u8; MAXDHLEN]);
    ...
    fn _write_message(&mut self, payload: &[u8], message: &mut [u8]) -> Result<usize, Error> {
        ...
        for token in self.message_patterns[self.pattern_position].iter() {
            match token {
                ...
                Token::Psk(n) => match self.psks[*n as usize].as_ref() {
                    Some(psk) => {
                ...
                Token::Ekem1 => {
                    ...
                    self.symmetricstate
                        .mix_key(&kem_output[..kem.shared_secret_len()]);
                }
            }
        }
    ...
    pub fn set_psk(&mut self, location: usize, key: &[u8]) -> Result<(), Error> {
        ...
        let mut new_psk = Zeroizing::new([0u8; PSKLEN]);
    ...
    pub fn dangerously_get_raw_split(&mut self) -> ([u8; CIPHERKEYLEN], [u8; CIPHERKEYLEN]) {
        ...
        self.symmetricstate
            .split_raw(&mut initiator_key, &mut responder_key);
        (
            initiator_key[..CIPHERKEYLEN].try_into().unwrap(),
            responder_key[..CIPHERKEYLEN].try_into().unwrap(),
        )
    }
