...
fn test_sanity_chachapoly_session() {
    ...
    let len = h_i
        .write_message(b"hack the planet", &mut buffer_msg)
        .unwrap();
    let len = h_r
...
fn test_Npsk0_chachapoly_expected_value() {
    ...
    let mut h_i = Builder::new(params)
        .remote_public_key(&x25519::x25519(
            get_inc_key(0),
...
fn test_NNhfs_sanity_session() {
    ...
    let params: NoiseParams = "Noise_NNhfs_25519+Kyber1024_ChaChaPoly_SHA256"
        .parse()
        .unwrap();
    let mut h_i = Builder::new(params.clone()).build_initiator().unwrap();
    let mut h_r = Builder::new(params).build_responder().unwrap();
...
fn test_handshake_message_receive_oversized_message() {
    ...
    assert_eq!(
        Error::Input,
        h_r.read_message(&buffer_msg, &mut buffer_out).unwrap_err()
    );
    h_r.read_message(&buffer_msg[..len], &mut buffer_out)
        .unwrap();
    ...
    let len = h_i
        .write_message(0, b"hack the planet", &mut buffer_msg)
        .unwrap();
    assert_eq!(
        Error::Input,
        h_r.read_message(&buffer_msg, &mut buffer_out).unwrap_err()
    ...
    assert_eq!(
        Error::Input,
        h_i.read_message(0, &buffer_msg, &mut buffer_out)
            .unwrap_err()
    );
    let len = h_i
        .read_message(0, &buffer_msg[..len], &mut buffer_out)
        .unwrap();
    assert_eq!(&buffer_out[..len], b"hack the planet");
}
...
fn test_oneway_responder_enforcements() {
    ...
    let mut resp = resp_builder
        .local_private_key(&rpk.private)
        .build_responder()
        .unwrap();
    let mut init = Builder::new(params)
        .remote_public_key(&rpk.public)
        .build_initiator()
        .unwrap();
...
fn test_checkpointing() {
    ...
    assert!(
        res.is_err(),
        "write_message() should have failed for insufficiently-sized buffer"
    );
    ...
    assert!(
        res.is_err(),
        "read_message() should have failed for insufficiently-sized buffer"
    );
...
fn test_stateless_sanity_session() {
    ...
    let len = h_i
        .write_message(1337, b"hack the planet", &mut buffer_msg)
        .unwrap();
    let len = h_r
        .read_message(1337, &buffer_msg[..len], &mut buffer_out)
        .unwrap();
    assert_eq!(&buffer_out[..len], b"hack the planet");
}
...
fn test_stateful_nonce_maxiumum_behavior() {
    ...
    let len = h_i
        .write_message(sender_nonce, b"xyz", &mut buffer_msg)
        .unwrap();
    ...
    sender_nonce += 1;
    let len = h_i
        .write_message(sender_nonce, b"abc", &mut buffer_msg)
        .unwrap();
