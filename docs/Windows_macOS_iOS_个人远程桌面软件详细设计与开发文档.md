...
> 维护说明：本文是早期跨平台产品设计与决策记录。当前实现和发布验收以[当前架构契约](current-architecture-contract.md)、[Windows 发布运行手册](windows-release-runbook.md)、当前源码与测试为准；本文中“第一版不做账号/文件”等历史范围不覆盖当前 Windows `0.1.91` 候选版已实现的可选账号、剪贴板和文件传输。
