...
遇到配对或连接问题时，进入“概览”中的“双机连接诊断”。界面会用稳定检查代码汇总当前状态：

- `DL-CFG-101`：连接设置是否由当前 Windows 用户安全保存；
- `DL-SEC-102`：可信设备存储是否可用；
- `DL-NET-201`：中继地址和 TLS 服务器名称是否可连接；
- `DL-NET-202`：是否发现可供另一台电脑访问的局域网地址；
- `DL-HOST-301`：主机运行组件当前是否正常。

点击“导出诊断报告”后，DeskLink 会把中文纯文本报告保存到当前用户的“下载”文件夹，并在界面显示报告编号、文件名和完整路径。报告不会包含会话 ID、中继密钥、设备公钥或完整设备身份，但会包含本机局域网 IP、网卡名称、检查结论和最多 200 条已脱敏的近期事件。将报告交给他人前仍应先检查内容，并只通过可信渠道发送。
