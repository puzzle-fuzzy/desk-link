...
DeskLink 的发布构建会按正确顺序处理两层可执行文件：

1. 构建并签名 `desklink-windows.exe`；
2. 把已签名的应用嵌入单文件安装器；
3. 构建并签名最终 `DeskLinkSetup-<version>-x64.exe`；
4. 对每个签名执行 Authenticode 发布策略和时间戳验证。
...
```powershell
python scripts/sign-windows-artifact.py dist\windows\DeskLinkSetup-0.1.0-x64.exe
```
