...
- 外部短时探针在完成目录和双向控制验证后会显式关闭临时 QUIC 连接；巡检中的容量样本不应因探针本身长期增加。
...
部署结果会回报实际加载镜像的源码 SHA；部署完成后还会重新读取运行中容器的 revision，确保 compose 没有启动错误的镜像。如果镜像缺少完整 40 位 revision，或运行中容器与加载镜像不一致，部署会自动回滚并失败。
