...
- DXGI 多显示器采集、屏幕切换与 Media Foundation H.264 实时编码；
...
当前明确不支持：整块虚拟桌面合成、远程音频、剪贴板同步、文件传输、自动更新，以及 Windows UAC 安全桌面和 `Ctrl+Alt+Delete` 注入。多显示器通过会话内切换当前屏幕实现。
...
5. 根据真实用户需求决定剪贴板、文件传输、音频和自动更新的优先级，并继续验证多显示器在不同 DPI 与排列下的坐标一致性。
