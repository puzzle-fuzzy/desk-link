...
- `server/account`：邮箱注册、验证、登录、会话刷新、退出和密码重置控制面；
...
0.1.79 继续使用协议 9；端到端数据报加密改为在带明文序列号前缀的单个缓冲区内完成，减少 ChaCha20-Poly1305 发送路径的中间密文分配和复制，同时保持密文格式、重放窗口和各安全 lane 隔离不变；公网中继无需升级。
...
0.1.82 继续使用协议 9；控制端复用远程光标的最新状态对象，由动画帧调度器提交最新位置，降低高频光标信号产生的短命对象数量；公网中继无需升级。

0.1.83 继续使用协议 9；控制端输入分发器为连续鼠标移动增加可复用待发送槽，IPC 忙碌时只保留最新位置，并在按钮、键盘等离散输入前自动按原顺序刷新；公网中继无需升级。
0.1.84 继续使用协议 9；清晰画质提高到 8 Mbps，并为 Windows Media Foundation H.264 编码器提交平均/峰值码率，让中文文字、细线和窗口边缘在保持 1080p/30 FPS 的前提下更清楚；公网中继无需升级。
0.1.85 继续使用协议 9；清晰档启用峰值约束 VBR、CABAC 和较高质量/速度档，同时修正适应模式全屏缩放盒，恢复全屏铺满并保持等比显示；公网中继无需升级。
0.1.86 使用协议 10；清晰档最高支持 2560 × 1440 / 12 Mbps，避免 2K 高 DPI 桌面先被主机压成 1080p；1:1 原始像素模式会在画面小于视口时居中、超出视口时保留滚动浏览；公网中继无需升级，但两台 Windows 必须同步升级。
0.1.87 继续使用协议 10；1:1 原始像素模式会根据 Windows 显示缩放比例调整 Canvas 的 CSS 尺寸，避免 WebView 在高 DPI 屏幕上二次插值；适应模式不受影响，公网中继无需升级。
0.1.88 延续协议 10；Windows 控制端采用编辑部式纸张界面：米白底、深蓝主动作、编号化连接区、细线网格和无阴影卡片；连接功能、设备保存和远程会话交互保持不变。
0.1.91 将开发协议统一升级到 12，加入 H.264 Main/High Profile 能力协商、按 Noise 会话派生的候选绑定和视频直连 Offer/Answer 状态机。Windows 控制端会先探测 High Profile，WebView2 不支持时自动请求主机切回 Main；主机编码器创建 High MFT 失败也会自动回退。同一局域网会阶段性尝试经过认证的 DirectLan 视频数据面，探测失败自动回落公网中继；跨网络仍使用中继，4K 仍未作为用户能力开放。项目尚未正式发布，开发中的两台 Windows 电脑使用同一套 0.1.91 构建。

## Windows 安装包

当前 `0.1.91` 仍是候选构建，已知限制与验证记录见 [CHANGELOG.md](CHANGELOG.md)。
...
Windows 敏感二进制存储均由 DPAPI 保护并原子替换。连接码、relay 加入密钥、私钥、完整设备身份和文件完整路径不会写入诊断日志或暴露给 WebView；配对批准与撤销保持系统原生、默认拒绝的确认边界。文件接收失败时会区分远端拒绝、传输占用、权限、磁盘空间或源文件变化，不会把所有失败统一解释为用户拒绝。断点文件只保留在“下载”文件夹同卷的隐藏暂存目录，续传前校验已有前缀哈希，完整哈希验证通过后才会发布；接收前还会为下载盘保留 64 MB 安全空间，显式取消、无效数据和超过 24 小时的残留会被清理。控制端的活动任务和等待队列恢复记录同样最多保留 24 小时，并用完整设备身份绑定原目标；活动任务只在用户连接同一设备并点击重试后恢复，等待队列只会暂停载入并要求用户点击“继续队列”，不会在启动、重连或连接其他设备时静默发送文件。放弃恢复只清除加密状态和匹配传输 ID 的隐藏断点，不删除用户原始文件；排队源文件在发送前还会重新校验名称、大小和修改时间。
...
3. 为公网 relay 增加第二节点、容量与证书告警，并执行故障切换演练；
4. 在代码签名证书就绪后启用正式 Release，继续实现经过独立更新签名验证的自动安装，并验证多显示器在不同 DPI、缩放模式与排列下的坐标一致性。
