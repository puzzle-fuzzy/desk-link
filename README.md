...
3. 在两台真实 Apple Silicon Mac 上完成 Screen Recording/Accessibility 授权、首次邀请配对和断网恢复验收。
...
## macOS Apple Silicon 桌面端

首次打开 `DeskLink.app` 时，选择本机在当前会话中的角色：

- **Control another Mac**：把这台 Mac 作为控制端。点击 **Paste invitation**，应用只从剪贴板读取 host 创建的 Base64 签名邀请，不会在界面中回显邀请内容。host 批准后，控制端会显示远端画面；获得焦点的画面会发送鼠标、滚轮和键盘输入。
- **Share this Mac**：把这台 Mac 作为 host。先在 **Screen Recording** 与 **Accessibility** 卡片中点 **Allow**；若系统没有立即授予权限，可点 **Open Settings**，在“隐私与安全性”中启用 DeskLink，然后返回应用并重新检查。Screen Recording 用于采集显示器，Accessibility 只用于把已批准控制端的输入应用到本机。

host 在两项权限均已授予后点击 **Create invitation**，再点击 **Copy invitation** 并仅通过可信渠道发送。邀请有到期时间；控制端粘贴后，host 会显示待批准控制端的指纹，必须明确批准才能建立会话。host 可以在 **Trusted controllers** 中撤销已批准设备；撤销后该设备不能继续作为已信任控制端连接。

DeskLink 将本机 host/controller 长期身份、可信控制端和已批准 host 的重连材料保存在当前用户的 Apple Keychain。控制端只有在 host 批准并成功建立连接后才保存该 host；之后 **Approved Macs** 列表中的 **Reconnect** 可使用该安全保存的材料重新连接。若 host 已更换身份、撤销控制端、邀请过期或连接凭据变更，应创建并使用新的邀请。

环境变量只保留给开发联调：`DESKLINK_RELAY_URL` 默认为 `quic://127.0.0.1:4433`，`DESKLINK_RELAY_SERVER_NAME` 默认为 `localhost`。正常桌面配对不需要设置 `DESKLINK_PAIRING_INVITE`、`DESKLINK_SESSION_ID`、`DESKLINK_AUTH_KEY` 或 `DESKLINK_HOST_VERIFY_KEY`。

在 Apple Silicon Mac 上构建并运行可重复检查：
...
`build-macos-arm64.sh --check` 会构建 `aarch64-apple-darwin` Rust FFI 和 arm64 Swift release executable，生成 `dist/macos/DeskLink.app`，并检查可执行文件架构、bundle identifier、Screen Recording 声明和最低系统版本。`verify-macos-runtime.sh` 另外运行 Rust FFI、local-relay fake-media end-to-end 与 Swift arm64 测试；它不会请求系统权限，也不会替代两台真实 Mac 的人工验收。
